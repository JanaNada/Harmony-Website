...
content = content.replace(
  ...
  function(match, p1) {
    if (!p1.includes('Search,')) {
      return "import { " + p1 + ", Search, ClipboardCheck, UsersRound, Repeat, BookOpen, GraduationCap, LineChart } from 'lucide-react';";
    }
    return match;
  }
);

// Replace RecruitmentPage
const recruitmentRegex = /function RecruitmentPage\(\{ go \}: \{ go: \(p: Page\) => void \}\) \{[\s\S]*?(?=function ContactPage)/;

const newRecruitmentPage = `function RecruitmentPage({ go }: { go: (p: Page) => void }) {
...
content = content.replace(recruitmentRegex, newRecruitmentPage + '\n\n');
